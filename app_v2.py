...
with tab_main:
    ...
    with col_up:
        ...
        uploaded_file = st.file_uploader(
            "Upload your Excel or CSV file (.xlsx / .csv)",
            type=["xlsx", "csv"],
    ...
    with st.expander("⬇️ Download sample Trial Balance file"):
        ...
        st.download_button(
            "⬇️ Download Sample (₹ Crores format — CSV)",
            data=csv_bytes,
            file_name="sample_trial_balance_crores.csv",
            mime="text/csv",
        )
        st.caption("Amounts are in ₹ Crores. Save as .xlsx in Excel before uploading, or upload the CSV directly.")
...
with tab_benford:
    ...
    with col_bf1:
        bf_file = st.file_uploader(
            "Upload transaction amounts file (.xlsx or .csv)",
            type=["xlsx", "csv"],
    ...
    with st.expander("⬇️ Download sample transaction file"):
        ...
        bf_csv = bf_sample.to_csv(index=False).encode("utf-8")
        st.download_button(
            "⬇️ Download Sample Transaction File (CSV)",
            data=bf_csv, file_name="sample_transactions_benford.csv",
            mime="text/csv"
        )
    ...
    if bf_file is not None:
        ...
        bf_suffix = ".csv" if bf_file.name.endswith(".csv") else ".xlsx"
        with tempfile.NamedTemporaryFile(suffix=bf_suffix, delete=False) as tmp_bf:
            tmp_bf.write(bf_file.read())
            bf_path = tmp_bf.name

        bf_df = pd.read_csv(bf_path) if bf_suffix == ".csv" else pd.read_excel(bf_path)
        # Find amount column
        amt_col = None
